...
## Load library
```{r loadinglibrary, cache=FALSE,message=FALSE}
library(ggplot2)
library(dplyr)
library(knitr)
library(lubridate)
...
``` {r meanandmedianofstepsperday_filleddata, results="asis"}
perDaySummary <- byDate %>% summarize(
        mean_step = mean(steps),
        median_step = median(steps)
    )
